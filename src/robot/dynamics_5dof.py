"""Dynamics for a spatial 5-DOF yaw-plus-planar robotic arm."""
...
from .kinematics_5dof import Arm5DOFConfig, ArrayLike5


@dataclass(frozen=True)
class Arm5DOFDynamicsConfig:
    """Physical constants for the 5-DOF manipulator.

    The dynamic model keeps the same modelling level as the 4-DOF simulator:
    a yaw inertia around the vertical axis and a rigid planar chain in the
    radial-z plane. The planar terms are assembled generically for 4 links.
    """

    arm_config: Arm5DOFConfig = field(default_factory=Arm5DOFConfig)
    link_masses: tuple[float, float, float, float] = (1.0, 0.8, 0.55, 0.35)
    center_of_mass_ratios: tuple[float, float, float, float] = (0.5, 0.5, 0.5, 0.5)
    link_inertias: tuple[float, float, float, float] | None = None
    base_yaw_inertia: float = 0.07
    gravity: float = 9.81
    viscous_friction: tuple[float, float, float, float, float] = (
        0.05,
        0.08,
        0.06,
        0.04,
    ...
    def __post_init__(self) -> None:
        ...
        if masses.shape != (4,) or np.any(masses <= 0.0):
            raise ValueError("link_masses must contain four positive values.")

        ratios = np.asarray(self.center_of_mass_ratios, dtype=float)
        if ratios.shape != (4,) or np.any(ratios <= 0.0) or np.any(ratios > 1.0):
            raise ValueError(
                "center_of_mass_ratios must contain four values in ]0, 1]."
            )

        if self.link_inertias is not None:
            inertias = np.asarray(self.link_inertias, dtype=float)
            if inertias.shape != (4,) or np.any(inertias <= 0.0):
                raise ValueError("link_inertias must contain four positive values.")
        ...
        if friction.shape != (5,) or np.any(friction < 0.0):
            raise ValueError("viscous_friction must contain five non-negative values.")


def _as_vector5(values: ArrayLike5, name: str) -> np.ndarray:
...
def yaw_inertia_and_gradient_5dof(
    q: ArrayLike5,
    config: Arm5DOFDynamicsConfig | None = None,
) -> tuple[float, np.ndarray]:
    """Return base yaw inertia and its gradient over q."""

    cfg = config or Arm5DOFDynamicsConfig()
    q_array = _as_vector5(q, "q")
    q_planar = q_array[1:]
...
def mass_matrix_5dof(
    ...
    cfg = config or Arm5DOFDynamicsConfig()
    q_array = _as_vector5(q, "q")
    yaw_inertia, _ = yaw_inertia_and_gradient_5dof(q_array, cfg)
...
def _mass_matrix_derivatives(
    ...
    derivatives = np.zeros((5, 5, 5), dtype=float)
    for index in range(1, 5):
        delta = np.zeros(5, dtype=float)
        delta[index] = epsilon
        derivatives[:, :, index] = (
            mass_matrix_5dof(q + delta, config)
            - mass_matrix_5dof(q - delta, config)
        ) / (2.0 * epsilon)
    return derivatives


def coriolis_centrifugal_torque_5dof(
    q: ArrayLike5,
    q_dot: ArrayLike5,
    config: Arm5DOFDynamicsConfig | None = None,
) -> np.ndarray:
    ...
    The current 5-DOF study keeps the dominant yaw/posture coupling and omits
    the smaller planar 4R Coriolis terms, as in the 4-DOF prototype.
    """

    cfg = config or Arm5DOFDynamicsConfig()
    q_array = _as_vector5(q, "q")
    velocity = _as_vector5(q_dot, "q_dot")
    torque = np.zeros(5, dtype=float)
    yaw_rate = float(velocity[0])
    _, yaw_gradient = yaw_inertia_and_gradient_5dof(q_array, cfg)
...
def viscous_friction_torque_5dof(
    q_dot: ArrayLike5,
    config: Arm5DOFDynamicsConfig | None = None,
) -> np.ndarray:
    ...
    cfg = config or Arm5DOFDynamicsConfig()
    return np.asarray(cfg.viscous_friction, dtype=float) * _as_vector5(
        q_dot,
        "q_dot",
    )


def inverse_dynamics_torque_5dof(
    q: ArrayLike5,
    q_dot: ArrayLike5,
    q_ddot: ArrayLike5,
    config: Arm5DOFDynamicsConfig | None = None,
    ...
    cfg = config or Arm5DOFDynamicsConfig()
    acceleration = _as_vector5(q_ddot, "q_ddot")
    return (
        mass_matrix_5dof(q, cfg) @ acceleration
        + coriolis_centrifugal_torque_5dof(q, q_dot, cfg)
        + gravity_torque_5dof(q, cfg)
        + viscous_friction_torque_5dof(q_dot, cfg)
    )


def joint_acceleration_5dof(
    q: ArrayLike5,
    q_dot: ArrayLike5,
    tau: ArrayLike5,
    config: Arm5DOFDynamicsConfig | None = None,
    ...
    cfg = config or Arm5DOFDynamicsConfig()
    torque = _as_vector5(tau, "tau")
    if external_torque is not None:
        torque = torque + _as_vector5(external_torque, "external_torque")
    ...
    return np.linalg.solve(mass_matrix_5dof(q, cfg), torque - passive)
