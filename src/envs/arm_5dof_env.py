"""Kinematic environment for the spatial 5-DOF arm."""
...
from robot.kinematics_5dof import ArrayLike3, ArrayLike5


Limit5 = float | Sequence[float] | np.ndarray


def _limit_vector5(value: Limit5, name: str) -> np.ndarray:
...
class Arm5DOFEnvConfig:
    """Configuration for the kinematic 5-DOF control environment."""

    arm_config: Arm5DOFConfig = field(default_factory=Arm5DOFConfig)
    target: tuple[float, float, float] = (1.25, 0.45, 0.60)
    dt: float = 0.05
    max_joint_speed: Limit5 = (2.0, 2.0, 2.0, 2.0, 2.0)
...
class Arm5DOFEnv:
    ...
    def __init__(self, config: Arm5DOFEnvConfig | None = None) -> None:
        self.config = config or Arm5DOFEnvConfig()
