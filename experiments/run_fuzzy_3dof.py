"""Run fuzzy velocity control on the 3-DOF kinematic arm."""
...
from envs import Arm3DOFEnv, Arm3DOFEnvConfig
from robot import inverse_kinematics_3dof
from visualization import plot_control_simulation_3dof


def main() -> int:
    config = Arm3DOFEnvConfig(
        target=(0.95, 0.55, 0.50),
        dt=0.05,
        max_joint_speed=(2.0, 2.0, 2.0),
        target_tolerance=1e-2,
        max_steps=500,
    )
    env = Arm3DOFEnv(config)
    ...
    controller = FuzzyVelocityController(
        error_scale=[0.45, 0.45, 0.75],
        derivative_scale=[4.0, 4.0, 4.0],
        output_scale=[2.0, 2.0, 2.0],
        size=3,
    ...
    output_path = ROOT / "results" / "figures" / "step_18_fuzzy_3dof.png"
    ...
    fig, _ = plot_control_simulation_3dof(
        ...
        title="Controle flou - bras 3DDL spatial",
    )
