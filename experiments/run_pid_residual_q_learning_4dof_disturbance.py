"""Learn a bounded torque residual for a 4-DOF arm under external disturbance."""
...
from rl import PID_RESIDUAL_ACTION_NAMES_4DOF, residual_acceleration_actions_4dof
from robot import inverse_dynamics_torque_4dof, inverse_kinematics_4dof


EXTERNAL_TORQUE = (0.0, -4.0, 0.0, 0.0)
RESIDUAL_TORQUE_SCALE = (0.1, 4.0, 2.0, 1.5)
EPISODES = 24
MAX_STEPS = 500
...
def _make_config() -> Arm4DOFDynamicEnvConfig:
    return Arm4DOFDynamicEnvConfig(
...
def _desired_q(config: Arm4DOFDynamicEnvConfig) -> np.ndarray:
...
def _rollout_fixed_residual(action_index: int) -> FixedResidualRollout:
    ...
    residual_actions = residual_acceleration_actions_4dof(RESIDUAL_TORQUE_SCALE)
...
def _train_bandit(seed: int = 57) -> BanditTrainingResult:
    rng = np.random.default_rng(seed)
    n_actions = len(PID_RESIDUAL_ACTION_NAMES_4DOF)
    ...
    with path.open("w", newline="", encoding="utf-8") as handle:
        ...
        for name, rollout in (
            ...
            writer.writerow(
                [
                    name,
                    PID_RESIDUAL_ACTION_NAMES_4DOF[rollout.action_index],
                    rollout.done,
                    rollout.truncated,
                    len(rollout.rewards),
        ...
        for index, value in enumerate(result.action_values):
            writer.writerow(
                [
                    PID_RESIDUAL_ACTION_NAMES_4DOF[index],
                    f"{value:.12e}",
...
def _write_markdown(
    ...
    lines = [
        ...
        f"Mode residuel : couple moteur borne, action apprise `{PID_RESIDUAL_ACTION_NAMES_4DOF[best_action]}`.",
        "",
    ...
    for name, rollout in (
        ...
        lines.append(
            "| "
            f"{name} | {PID_RESIDUAL_ACTION_NAMES_4DOF[rollout.action_index]} | "
    ...
    for index, value in enumerate(result.action_values):
        lines.append(
            f"| {PID_RESIDUAL_ACTION_NAMES_4DOF[index]} | "
...
def _save_plot(
    ...
    x = np.arange(len(PID_RESIDUAL_ACTION_NAMES_4DOF))
    ax_values.bar(x, result.action_values)
    ax_values.set_xticks(x)
    ax_values.set_xticklabels(PID_RESIDUAL_ACTION_NAMES_4DOF, rotation=30, ha="right")
    ...
    fig.suptitle("RL residuel borne sous perturbation externe - bras 4DDL")
    fig.tight_layout()
    path.parent.mkdir(parents=True, exist_ok=True)
...
def main() -> int:
    ...
    table_path = ROOT / "results" / "tables" / "step_27_pid_residual_disturbance_4dof.csv"
    markdown_path = ROOT / "results" / "tables" / "step_27_pid_residual_disturbance_4dof.md"
    figure_path = ROOT / "results" / "figures" / "step_27_pid_residual_disturbance_4dof.png"
    ...
    print(f"residual_mode=torque_bandit")
    print(f"episodes={EPISODES}")
    print(f"best_action={PID_RESIDUAL_ACTION_NAMES_4DOF[best_action]}")
