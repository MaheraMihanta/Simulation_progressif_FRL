"""Train fuzzy residual Q-learning on the dynamic 2-DOF arm."""
...
def _plot_learning(ax, returns: np.ndarray, success: np.ndarray) -> None:
    x_return, mean_return = _moving_average(returns, window=25)
    x_success, success_rate = _moving_average(success.astype(float), window=25)
    ax.plot(x_return + 1, mean_return, color="tab:blue", label="return moyen")
    ax.set_title("Apprentissage Q sur regles floues")
...
def _format_rule(encoder: FuzzyDynamicStateEncoder, rule_index: int) -> str:
    terms = encoder.rule_terms(rule_index)
    return ", ".join(
        f"{variable}={term}"
        for variable, term in zip(FUZZY_STATE_VARIABLES, terms)
    )
...
def main() -> int:
    ...
    baseline_rollout = rollout_fuzzy_residual_q_policy(
        env_config,
        zero_q_value,
        encoder,
        config=learning_config,
    ...
    output_path = ROOT / "results" / "figures" / "step_10_fuzzy_residual_q_learning_2dof.png"
    ...
    fig.suptitle("Q-learning residuel avec etats flous - bras 2DDL")
    fig.tight_layout()
