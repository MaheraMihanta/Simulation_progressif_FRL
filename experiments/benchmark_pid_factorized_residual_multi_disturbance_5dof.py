"""Evaluate factorized residual actions under multi-joint 5-DOF disturbances."""
...
MAX_STEPS = 350
RESIDUAL_TORQUE_SCALE = (0.1, 4.0, 3.0, 1.0, 1.0)
    ...
    for _ in range(COORDINATE_SEARCH_PASSES):
        for joint in range(5):
            ...
            for local_action in range(LOCAL_ACTION_COUNT):
                ...
                residual_torque = factorized_residual_acceleration_action_5dof(
                    candidate_actions,
...
def _write_markdown(
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    evaluations = 5 * LOCAL_ACTION_COUNT * COORDINATE_SEARCH_PASSES
    lines = [
        "# Actions residuelles factorisees 5DDL",
        "",
        "Recherche locale : "
        f"`5 articulations x 3 choix x {COORDINATE_SEARCH_PASSES} passes = {evaluations}` "
        "evaluations par scenario, sans enumeration du produit cartesien `3^5 = 243`.",
        "",
...
def _save_plot(
    ...
    fig.suptitle("Actions residuelles factorisees - bras 5DDL")
    fig.tight_layout()
    path.parent.mkdir(parents=True, exist_ok=True)
...
def main() -> int:
    ...
    table_path = ROOT / "results" / "tables" / "step_35_factorized_residual_5dof.csv"
    markdown_path = ROOT / "results" / "tables" / "step_35_factorized_residual_5dof.md"
    figure_path = ROOT / "results" / "figures" / "step_35_factorized_residual_5dof.png"
    ...
    print(
        ...
        f"{5 * LOCAL_ACTION_COUNT * COORDINATE_SEARCH_PASSES}"
    )
    print("cartesian_actions_avoided=243")
    for scenario, baseline, best_axis, factorized in summaries:
        print(
