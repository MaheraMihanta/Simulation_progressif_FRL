"""Benchmark PID and fuzzy gain-scheduled PID on several 5-DOF targets."""
...
TARGETS = (
    ("T1_reference", (1.25, 0.45, 0.60)),
    ("T2_lateral_bas", (1.55, -0.35, 0.35)),
    ("T3_haut_diagonal", (0.90, 0.90, 0.80)),
    ("T4_avant_droit", (1.35, 0.75, -0.20)),
    ("T5_arriere_haut", (0.75, -1.05, 0.65)),
)
...
def _save_plot(path: Path, results: list[BenchmarkResult]) -> None:
    ...
    fig.suptitle("Comparaison multi-cibles PID vs PID a gains flous - 5DDL")
    fig.tight_layout()
    path.parent.mkdir(parents=True, exist_ok=True)
...
def main() -> int:
    ...
    table_path = ROOT / "results" / "tables" / "step_30_pid_vs_fuzzy_gain_5dof.csv"
    markdown_path = ROOT / "results" / "tables" / "step_30_pid_vs_fuzzy_gain_5dof.md"
    figure_path = ROOT / "results" / "figures" / "step_30_pid_vs_fuzzy_gain_5dof.png"
