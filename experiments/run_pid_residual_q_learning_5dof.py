"""Train residual Q-learning on top of fuzzy gain-scheduled PID for 5-DOF."""
...
from envs import Arm5DOFDynamicEnvConfig
from rl import (
...
from visualization import plot_control_simulation_5dof
...
def _save_learning_plot(
    ...
    fig.suptitle("Q-learning residuel sur PID adapte - bras 5DDL")
    fig.tight_layout()
...
def main() -> int:
    ...
    encoder = PIDResidualStateEncoder5DOF(
        joint_error_deadband=(0.04, 0.05, 0.05, 0.05, 0.05),
    ...
    learning_config = PIDResidualQLearning5DOFConfig(
        ...
        epsilon_end=0.05,
        epsilon_decay=0.965,
        residual_acceleration_scale=(0.30, 0.40, 0.40, 0.30, 0.25),
        seed=47,
    )
    ...
    learned_rollout = rollout_pid_residual_q_policy_5dof(
        env_config,
        result.q_value,
        encoder,
        config=learning_config,
        desired_q=result.desired_q,
        safety_config=PIDResidualSafety5DOFConfig(patience=80, min_progress=1e-4),
    )
    zero_q_value = np.zeros_like(result.q_value)
    baseline_rollout = rollout_pid_residual_q_policy_5dof(
        env_config,
        zero_q_value,
        encoder,
        config=learning_config,
    ...
    output_path = ROOT / "results" / "figures" / "step_31_pid_residual_q_learning_5dof.png"
    learning_output_path = (
        ROOT / "results" / "figures" / "step_31_pid_residual_q_learning_5dof_learning.png"
    )
    output_path.parent.mkdir(parents=True, exist_ok=True)

    fig, _ = plot_control_simulation_5dof(
        ...
        title="PID adapte + RL residuel - bras 5DDL spatial",
        action_ylabel="N.m",
    )
    fig.savefig(output_path, dpi=150)
    _save_learning_plot(
    ...
    learned_actions = [
        PID_RESIDUAL_ACTION_NAMES_5DOF[action] for action in learned_rollout.action_indices
    ]
    residual_actions = residual_acceleration_actions_5dof(
        learning_config.residual_acceleration_scale
    )

    print(f"state_count={encoder.n_states}")
    print(f"action_count={len(PID_RESIDUAL_ACTION_NAMES_5DOF)}")
